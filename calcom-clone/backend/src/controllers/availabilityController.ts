...
function validateAvailabilityInput(timezone: unknown, rules: unknown) {
  ...
  for (const rule of rules as AvailabilityRuleInput[]) {
    if (
      ...
      (rule.dayOfWeek as number) < 0 ||
      (rule.dayOfWeek as number) > 6
    ) {
    ...
    if (
      timeToMinutes(rule.startTime as string) >=
      timeToMinutes(rule.endTime as string)
    ) {
...
// Old: 9 AM – 5 PM
// New: 10 AM – 4 PM
export const upsertAvailability = asyncHandler(
  async (req: Request, res: Response) => {
    const user = (req as any).user;
